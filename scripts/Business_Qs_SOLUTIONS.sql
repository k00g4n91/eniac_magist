...
-- table of all delivery_times
SELECT 
	ROUND(SUM((order_delivered_customer_date - order_purchase_timestamp)/1000/60/60/24), 2) AS delivery_time_days
FROM orders;

-- ANSWER!!!
SELECT 
	ROUND(AVG((order_delivered_customer_date - order_purchase_timestamp)/1000/60/60/24), 2) AS avg_delivery_time_days 
FROM orders;
